...
| 2026-03-03 | Simplified normalization payload to six fields and standardized `unit_type` to uppercase SEFAZ-like codes | Reduce AI variance/noise and align normalization with invoice-native unit semantics | `quantity` + invoice unit remain in `items`; normalization stores only naming/brand/unit confidence fields | Team | If package-size modeling becomes necessary for mature price-per-unit analytics |
